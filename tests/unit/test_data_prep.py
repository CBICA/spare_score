...
from data_prep import check_test, check_train, smart_unique, age_sex_match, logging_basic_config, convert_cat_variables # If updates go through, it can be updated to spare_scores.data_prep
...
class CheckDataPrep(unittest.TestCase):
    ...
    def test_smart_unique(self):
        ...
        self.df_fixture = load_df("../fixtures/sample_data.csv")
        result = smart_unique(self.df_fixture, None, None)
        ...
        correct_df = {
            ...
            "ScanID": ["Scan001", "Scan002", "Scan003", "Scan004", "Scan005", "Scan006"],
            "Age": [35, 40, 45, 31, 45, 45],
            "Sex": ["M", "F", "F", "M", "F", "F"],
            "ROI1": [0.64, 0.64, 0.64, 0.64, 0.64, 0.84],
            "ROI2": [0.73, 0.91, 0.64, 0.76, 0.78, 0.73],
            "ID": [float('nan'), float('nan'), float('nan'), float('nan'), float('nan'), 5.0]
        }
    ...
    def test_age_sex_match(self):
        ...
        self.df_fixture = load_df("../fixtures/sample_data.csv")
        result = age_sex_match(self.df_fixture, None, None)
        ...
        result = age_sex_match(self.df_fixture1, self.df_fixture2, to_match=None, age_out_percentage=150) # Now no_df2=False
        ...
        self.df_fixture1 = pd.DataFrame(data=df1)
        self.df_fixture2 = self.df_fixture1
        result = age_sex_match(self.df_fixture1, self.df_fixture2, to_match=None) # here sex_match=False
        self.assertTrue(result == (self.df_fixture1, self.df_fixture2))
    ...
    def test_logging_basic_config(self):
        ...
        logging.basicConfig(level=logging_level[1], format=fmt, force=True, filename=filename) 
