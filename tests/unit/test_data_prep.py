...
def test_check_train():
    ...
    filtered_df, filtered_predictors, mdl_type = check_train(
        df_fixture, predictors, to_predict, pos_group=pos_group
    )
    assert(
        filtered_df.equals(df_fixture)
...
def test_smart_unique():
    ...
    df_fixture = load_df("../fixtures/sample_data.csv")
    result = smart_unique(df_fixture, None)
    ...
    result = smart_unique(df_fixture, None, to_predict="ROI1")
    err_msg = "Variable to predict has no variance."
    assert(result == err_msg)

    # test case 3: testing smart_unique with variance and no duplicate ID's. df2=None
    df_fixture = load_df("../fixtures/sample_data.csv")
    result = smart_unique(df_fixture, None, "ROI1")
    assert(result.equals(df_fixture))
    ...
    df_fixture = df_fixture._append(new_row, ignore_index=True)
    result = smart_unique(df_fixture, None, "ROI1")
    correct_df = {
    ...
    assert(result.equals(correct_df))
    ...
    df_fixture1 = pd.DataFrame(data=df1)
    df_fixture2 = pd.DataFrame(data=df2)

    result = smart_unique(df_fixture1, df_fixture2, to_predict=None)
    assert(result == (df_fixture1, df_fixture2))

def test_age_sex_match():
    # test case 1: testing df2=None and to_match=None
    df_fixture = load_df("../fixtures/sample_data.csv")
    result = age_sex_match(df_fixture, None)
    ...
    df_fixture = pd.DataFrame(data=df)
    result = age_sex_match(df_fixture, None, to_match="Var2")
    err_msg = "Variable to match must be binary"
    assert(result == err_msg)

    # test case 3: testing df2!=None and to_match=None with age_out_percentage>=100
    df_fixture1 = load_df("../fixtures/sample_data.csv")
    df_fixture2 = df_fixture1
    result = age_sex_match(
        df_fixture1, df_fixture2, to_match=None, age_out_percentage=150
    )  # Now no_df2=False
    ...
    df_fixture2 = df_fixture1
    result = age_sex_match(
        df_fixture1, df_fixture2, to_match=None
    ...
    df_fixture1 = pd.DataFrame(data=df1)
    result = age_sex_match(df_fixture1, None, to_match="Sex")
...
def test_logging_basic_config():
    ...
    assert(os.path.exists("test_data_prep_notexisting.py"))
    assert(result == logging.getLogger())
    os.remove(filename)
    ...
    assert(os.path.exists("test_data_prep.py"))
    assert(result == logging.getLogger())
