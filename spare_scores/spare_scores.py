...
def spare_train(
    ...
    # Load the data, and check if it contains any errors.
    df = _load_df(df)
    ...
    if len(df.index) > 1000:
        logging.info('Due to large dataset, first performing parameter tuning '
...
def spare_test(df: Union[pd.DataFrame, str],
    ...
    for var, map_dict in meta_data.get('categorical_var_map',{}).items():
        ...
        if df[var].isin(map_dict.keys()).any():
            ...
            logging.error(f'Column "{var}" expected {expected_var}, but '
    ...
    logging.info(f'Model Info: training N = {n} / ages = {a1} - {a2} / '
