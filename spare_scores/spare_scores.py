...
def spare_train(df: Union[pd.DataFrame, str], 
    ...
    Args:
        ...
        predictors: a list of predictors for the training. All must be present 
                    in columns of df.
    ...
    Raises:
        ValueError: If the predictor set of variables contain any categorical
                    variable that contains more than 2 categories.
    """
    logging_basic_config(verbose)

    # Load the data, check if df contains errors. 
    df = _load_df(df)
    ...
    categorical_vars = [var for var in df[predictors].columns 
                       if df[var].dtype == 'O']
    meta_data.categorical_var_map = {var: None for var in categorical_vars}

    # Convert them to integers:
    for var in categorical_vars:
        if len(df[var].unique()) <= 2:
            categorical_var_mapping = {}
            for count, categorical_var in enumerate(df[var].unique()):
                # enum count starts at 0 -> want to start from 1.
                categorical_var_mapping[categorical_var] = count + 1
            meta_data.categorical_var_map[var] = categorical_var_mapping
